...
export default function StoryViewer({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-black">
      ...
      {/* Content */}
      <div className="flex-1 flex items-center justify-center" style={{ background: story.bg_color || '#111' }}>
        {story.type === 'image' && <img src={story.media_url} alt="" className="max-h-full max-w-full object-contain" />}
        {story.type === 'video' && <video src={story.media_url} autoPlay muted playsInline className="max-h-full max-w-full" />}
        {story.type === 'text' && (
      ...
      <div className="absolute inset-0 z-10 flex">
        <button className="w-1/2 h-full" aria-label="Previous story" onClick={() => {
        ...
        <button className="w-1/2 h-full" aria-label="Next story" onClick={() => {
          const feed = statusFeedRef.current
